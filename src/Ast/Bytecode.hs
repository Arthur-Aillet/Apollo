...
toBytecode' (Cast x y) = (instructionBytecode ECast, 2, [indexBytecode x, typeBytecode y])
...
fromBytecode' ECast 2 (x : y : xs) = Right (Cast (bytecodeIndex x) (bytecodeType y), xs)
